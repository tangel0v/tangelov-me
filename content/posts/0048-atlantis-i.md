...
La única forma de solventar esto es mediante el uso de instalaciones privadas de Atlantis paracada proyecto o integrándolo con Terraform Cloud (que proporciona muchos de los problemas aquí comentados). En el ejemplo de este post, tenemos que definir las variabls de entorno directamente en Atlantis. Si tenemos múltiples proyectos y entornos por proyecto, no vamos a poder escalarlo y va a ser necesario gestionar los permisos de Terraform de forma diferente. En un próximo post hablaré específicamente de este tema.
